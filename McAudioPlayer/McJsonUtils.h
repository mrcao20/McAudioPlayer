#ifndef _MC_JSON_UTILS_H_
#define _MC_JSON_UTILS_H_
...
struct McJsonTypeSelector<T *, QMetaType::PointerToQObject> {
	static QJsonValue toJsonValue(T *value) {
		return QJsonValue(McJsonUtils::objToJsonObj(qobject_cast<QObject *>(value)));
	}
};
template<typename T>
struct McJsonTypeSelector<T, QMetaType::IsGadget> {
	static QJsonValue toJsonValue(const T &value) {
		return QJsonValue(McJsonUtils::gadgetToJsonObj(static_cast<void*>(&value), T::staticMetaObject));
	}
};
template<typename T>
...
public:
	static QString objToJson(QObject *obj) noexcept;
	static QJsonObject objToJsonObj(QObject *obj) noexcept;
	static QJsonObject gadgetToJsonObj(void *gadget, const QMetaObject *mobj) noexcept;

	template<typename T>
	static QString responseToJson(const McRestResponse<T> &response) noexcept;
...
template<typename T>
QString McJsonUtils::responseToJson(const McRestResponse<T> &response) noexcept {
	QJsonObject jsonObj;
